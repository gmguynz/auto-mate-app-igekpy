...
export const storageUtils = {
  ...
  async getCustomers(): Promise<Customer[]> {
    return performanceMonitor.track('getCustomers', async () => {
      try {
  ...
  async saveCustomers(customers: Customer[]): Promise<void> {
    return performanceMonitor.track('saveCustomers', async () => {
      try {
        if (this.shouldUseSupabase()) {
  ...
  async addCustomer(customer: Customer): Promise<void> {
    return performanceMonitor.track('addCustomer', async () => {
      try {
        if (this.shouldUseSupabase()) {
  ...
  async updateCustomer(updatedCustomer: Customer): Promise<void> {
    return performanceMonitor.track('updateCustomer', async () => {
      try {
        if (this.shouldUseSupabase()) {
  ...
  async deleteCustomer(customerId: string): Promise<void> {
    return performanceMonitor.track('deleteCustomer', async () => {
      try {
        if (this.shouldUseSupabase()) {
  ...
  async getCustomerById(customerId: string): Promise<Customer | null> {
    return performanceMonitor.track('getCustomerById', async () => {
      try {
        if (this.shouldUseSupabase()) {
  ...
  async migrateToSupabase(): Promise<{ success: boolean; message: string }> {
    return performanceMonitor.track('migrateToSupabase', async () => {
      try {
        if (!this.shouldUseSupabase()) {
          return {
            success: false,
